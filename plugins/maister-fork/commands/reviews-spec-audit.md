---
name: maister-fork:reviews-spec-audit
description: Independent specification audit to verify completeness and clarity before implementation
---

**ACTION REQUIRED**: This command delegates to a different skill. The `<command-name>` tag refers to THIS command, not the target. Call the Task tool with subagent_type="maister-fork:spec-auditor" NOW. Pass the spec path in the prompt. Do not read files, explore code, or execute workflow steps yourself.
...
```
User: /maister-fork:reviews-spec-audit .maister/tasks/development/2025-11-17-user-auth/implementation/spec.md
```

**Example 2**: Post-implementation audit
```
User: /maister-fork:reviews-spec-audit .maister/tasks/development/2025-11-17-user-auth/ --post-implementation
```

**Example 3**: Audit with clarification focus
```
User: /maister-fork:reviews-spec-audit spec.md --focus=ambiguity
```
