---
name: maister-fork:reviews-reality-check
description: Comprehensive reality assessment of completed work to verify it actually works and is production-ready
---

**ACTION REQUIRED**: This command delegates to a different skill. The `<command-name>` tag refers to THIS command, not the target. Call the Task tool with subagent_type="maister-fork:reality-assessor" NOW. Pass the task path in the prompt. Do not read files, explore code, or execute workflow steps yourself.
...
```
User: /maister-fork:reviews-reality-check .maister/tasks/development/2025-11-17-payment-processing/
```

**Example 2**: Verify claimed completion
```
User: /maister-fork:reviews-reality-check .maister/tasks/development/2025-11-17-login-timeout/
```

**Example 3**: Production readiness check
```
User: /maister-fork:reviews-reality-check .maister/tasks/development/2025-11-17-user-dashboard/ --production
```
