...
0. For every group in the wave, `TaskUpdate` to `status: "in_progress"` with `owner: "maister-fork:task-group-implementer"`.
...
2. **Fan out — CRITICAL: parallel dispatch in a single message**:
   ...
   Per-call parameters:
   - subagent_type: `maister-fork:task-group-implementer`
