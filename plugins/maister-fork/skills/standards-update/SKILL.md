---
name: maister-fork:standards-update
...
```bash
/maister-fork:standards-update                                    # Detect from conversation
/maister-fork:standards-update "always use React.memo for lists"  # From description
/maister-fork:standards-update --from=/path/to/other-project      # Sync from another project
```
...
3. Check `.maister/docs/standards/` exists in the current project. If not, offer to run `/maister-fork:init` first.
...
Invoke `docs-operator` subagent via Task tool (subagent_type: `maister-fork:docs-operator`):
...
**Invoke `docs-operator` subagent** via Task tool (subagent_type: `maister-fork:docs-operator`) with context:
...
If `.maister/docs/` doesn't exist, offer to run `/maister-fork:init` first.
