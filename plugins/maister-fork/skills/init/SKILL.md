...
**Invoke `docs-operator` subagent** via Task tool (subagent_type: `maister-fork:docs-operator`) with prompt:
...
# (development Phase 4, product-design Phase 7, or /maister-fork:mockup-studio).
...
**Step 1**: Invoke `docs-operator` subagent via Task tool (subagent_type: `maister-fork:docs-operator`) with prompt:
...
- Next steps:
  ...
  3. Start development with `/maister-fork:work`
