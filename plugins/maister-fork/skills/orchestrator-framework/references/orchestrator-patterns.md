...
|--------------|----------------|------------------|
| "I'll analyze the codebase..." | Bypasses codebase-analyzer skill | Use `Skill` tool with `maister-fork:codebase-analyzer` |
| "Let me create the specification..." | Bypasses specification-creator | Use `Task` tool with `maister-fork:specification-creator` subagent |
| "Looking at the gaps between..." | Bypasses gap-analyzer subagent | Use `Task` tool with `maister-fork:gap-analyzer` |
| "I'll implement this by..." | Bypasses implementation-plan-executor skill | Use `Skill` tool with `maister-fork:implementation-plan-executor` |
...
- Specification creation → always delegate to `maister-fork:specification-creator` subagent
- Implementation planning → always delegate to `maister-fork:implementation-planner` subagent
- Gap analysis → always delegate to `maister-fork:gap-analyzer` subagent
- Codebase analysis → always delegate to `maister-fork:codebase-analyzer` skill
- Code review → always delegate to `maister-fork:code-reviewer` subagent
- Test execution → always delegate to `maister-fork:test-suite-runner` subagent
- Implementation completeness → always delegate to `maister-fork:implementation-completeness-checker` subagent
...
An optional project-level config file at `.maister/config.yml` (sibling of `.maister/docs/` and `.maister/tasks/`) holds defaults that apply to every workflow. It is scaffolded by `/maister-fork:init` but is not required — when absent, every key falls back to its default.
