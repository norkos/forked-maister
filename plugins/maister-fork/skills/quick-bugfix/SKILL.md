---
name: maister-fork:quick-bugfix
...
For complex bugs that grow beyond a quick fix, suggests escalating to the full development workflow (`/maister-fork:development`).

## Usage

```bash
/maister-fork:quick-bugfix "Login form submits twice on slow connections"
/maister-fork:quick-bugfix "API returns 500 when email contains special characters"
/maister-fork:quick-bugfix "Dark mode toggle doesn't persist after refresh"
```

## When to Use

**Use `/maister-fork:quick-bugfix` when:**
...
**Use `/maister-fork:development` instead when:**
...
If `.maister/docs/INDEX.md` does not exist, note it and suggest `/maister-fork:init` in the completion summary.
...
- Options:
  ...
  2. "Switch to full development workflow" — stop here and suggest running `/maister-fork:development` with the bug description and analysis context
...
If no standards exist: "No Maister standards found. Consider running `/maister-fork:init`."]
...
- Suggest escalating to `/maister-fork:development` for a more thorough approach
...
```
"No Maister standards found. Consider running `/maister-fork:init` to initialize
