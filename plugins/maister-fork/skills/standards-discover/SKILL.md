---
name: maister-fork:standards-discover
...
2. **Check prerequisites**: Verify `.maister/docs/` exists. If not, offer to run `/maister-fork:init` first
...
3. **Invoke `docs-operator` subagent** via Task tool (subagent_type: `maister-fork:docs-operator`) — Pass prepared content. For creates: new file. For updates: merge new findings with existing. Wait for completion, then continue with the next standard.
...
| Situation | Strategy |
|-----------|----------|
| `.maister/docs/` missing | Offer `/maister-fork:init`, abort if declined |
...
/maister-fork:standards-discover

# Quick scan (config files only, ~30-60s)
/maister-fork:standards-discover --scope=quick

# Frontend standards only
/maister-fork:standards-discover --scope=frontend

# High confidence, auto-apply
/maister-fork:standards-discover --confidence=80 --auto-apply

# Skip external analysis (offline/no GitHub)
/maister-fork:standards-discover --skip-external
```
