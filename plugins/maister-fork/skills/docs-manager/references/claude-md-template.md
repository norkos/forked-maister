...
Before writing or changing any code — even for quick, direct requests that don't go through a `/maister-fork:*` workflow — ground yourself in the project's documentation:
...
When this happens, briefly suggest the standard to the user. If approved, invoke `/maister-fork:standards-update` with the identified pattern.

## Maister Workflows

This project uses the maister plugin for structured development workflows. When any `/maister-fork:*` command is invoked, execute it via the Skill tool immediately — do not skip workflows for "straightforward" tasks. The user chose the workflow intentionally; complexity assessment is the workflow's job.
```
