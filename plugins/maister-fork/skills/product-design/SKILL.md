---
name: maister-fork:product-design
...
**DO NOT use for**: Implementation tasks (use `/maister-fork:development`), pure research (use `/maister-fork:research`), bug fixes, performance optimization.
...
> Phase 7 (Visual Prototyping) delegates to the `maister-fork:mockup-studio` skill, which owns the visual companion server and its reference docs (`skills/mockup-studio/references/visual-companion.md`, `design-resource-discovery.md`).
...
1. Skill tool - `maister-fork:codebase-analyzer` (to understand existing product context, tech stack, UI patterns)

**SELF-CHECK**: Did you invoke the Skill tool with `maister-fork:codebase-analyzer`? Or did you start reading project files yourself? If the latter, STOP and invoke the Skill tool.
...
4. If `design_context.research_topics` is non-empty: launch information-gatherer agents for each topic
   ...
   **INVOKE NOW** -- Task tool call (parallel, one per topic):
   Task tool - `maister-fork:information-gatherer` subagent per research topic
...
Task tool - `maister-fork:solution-brainstormer` subagent
...
7. **HTML companion** *(skip when `options.html_output` is false)*: invoke `maister-fork:html-companion-writer` (Task tool) for `analysis/design-decisions.md` (see Operator Visibility § 3). Register the returned `html_path` in `phase_summaries.idea_convergence.artifacts`.
...
**Execute**: Skill tool - `maister-fork:mockup-studio`
...
**INVOKE NOW** — Skill tool - `maister-fork:mockup-studio` with:
...
/maister-fork:development [task-path]
```
...
Invoked via:
- `/maister-fork:product-design [description] [--no-visual] [--research=PATH]` (new)
- `/maister-fork:product-design [task-path] [--from=PHASE]` (resume)
...
```
/maister-fork:development .maister/tasks/product-design/YYYY-MM-DD-task-name/
```
...
```
/maister-fork:product-design "Design feature X" --research=.maister/tasks/research/YYYY-MM-DD-research/
```
