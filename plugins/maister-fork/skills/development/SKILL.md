...
**Execute**:
1. Skill tool - `maister-fork:codebase-analyzer`
...
**Execute**:
1. Task tool - `maister-fork:gap-analyzer` subagent
...
6. Task tool - `maister-fork:specification-creator` subagent
...
**SELF-CHECK**: Did you just invoke the Task tool with `maister-fork:specification-creator`? Or did you start writing spec.md yourself? If the latter, STOP immediately and invoke the Task tool instead.
...
**Execute**: Task tool - `maister-fork:spec-auditor` subagent
...
**Execute**: Task tool - `maister-fork:implementation-planner` subagent
...
**SELF-CHECK**: Did you just invoke the Task tool with `maister-fork:implementation-planner`? Or did you start writing implementation-plan.md yourself? If the latter, STOP immediately and invoke the Task tool instead.
...
**Execute**: Skill tool - `maister-fork:implementation-plan-executor`
...
**SELF-CHECK**: Did you just invoke the Skill tool with `maister-fork:implementation-plan-executor`? Or did you start writing code yourself? If the latter, STOP immediately and invoke the Skill tool instead.
...
**Step 1**: Invoke Skill tool - `maister-fork:implementation-verifier`
...
5. AskUserQuestion — "Re-run verification to check fixes?" with options:
   - "Yes, re-run verification" → re-invoke `maister-fork:implementation-verifier` → return to Step 2
...
```
/maister-fork:development .maister/tasks/product-design/2026-05-09-user-dashboard/
```
...
```
/maister-fork:development "Implement the dashboard from /tmp/dashboard-mockup.html"
```
...
Invoked via:
- `/maister-fork:development [description] [--research=PATH]` (new)
- `/maister-fork:development [task-path] [--from=PHASE] [--reset-attempts]` (resume)
