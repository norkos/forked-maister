...
**Execute**: Task tool - `maister-fork:bottleneck-analyzer` subagent
...
4. Task tool - `maister-fork:bottleneck-analyzer` subagent

**Context to pass**: task_path, description, codebase analysis summary from Phase 1, user data paths (if any)

**SELF-CHECK**: Did you just invoke the Task tool with `maister-fork:bottleneck-analyzer`? Or did you start analyzing code yourself? If the latter, STOP and invoke the Task tool.
...
4. Task tool - `maister-fork:specification-creator` subagent
...
**SELF-CHECK**: Did you just invoke the Task tool with `maister-fork:specification-creator`? Or did you start writing spec.md yourself? If the latter, STOP and invoke the Task tool.
...
**SELF-CHECK**: Did you just invoke the Task tool with `maister-fork:implementation-planner`? Or did you start writing the plan yourself? If the latter, STOP and invoke the Task tool.
...
6. If re-run → re-invoke `maister-fork:implementation-verifier` → return to Step 2
...
Invoked via:
- `/maister-fork:performance [description] [--sequential]` (new)
- `/maister-fork:performance [task-path] [--from=PHASE] [--sequential]` (resume)
