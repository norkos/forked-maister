---
name: maister-fork:mockup-studio
...
- **Invoked standalone by a user** (`/maister-fork:mockup-studio "<screen or feature>"`). It creates its own task directory and runs the full interactive flow.
...
When a user runs `/maister-fork:mockup-studio "<screen or feature>"` (no `task_path` from an orchestrator):
