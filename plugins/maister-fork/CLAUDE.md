...
> Security-hardened fork of Skillpanel's Maister plugin (upstream version 2.2.3, [SkillPanel/maister](https://github.com/SkillPanel/maister)). Plugin and marketplace name: `maister-fork`. Commands are invoked as `/maister-fork:*`. The `.maister/` project directory name is unchanged from upstream.
...
UI mockups are generated by the shared **`mockup-studio`** skill, used by both the development workflow (Phase 4) and the product-design workflow (Phase 7), and invocable standalone (`/maister-fork:mockup-studio`). It runs in the main agent context, so it can drive a live visual-companion server, a browser preview, and interactive refinement gates.
...
- Standalone — `/maister-fork:mockup-studio "<screen or feature>"`
...
```
.maister/
├── config.yml                    # Project configuration (optional; scaffolded by /maister-fork:init)
...
An optional project-level config file holds defaults that apply to every workflow. `/maister-fork:init` scaffolds it with documented defaults; orchestrators read it at initialization and fall back to defaults when it is absent (so existing projects are unaffected).
...
- **Version Control**: `/maister-fork:init` adds `.maister/tasks/` to the project's `.gitignore` so task artifacts (analyses, specs, verification reports) stay local; `.maister/docs/` remains tracked
...
| `maister-fork:init` | Initialize `.maister/docs/` with project analysis, documentation generation, and baseline standards | `skills/init/SKILL.md` |
...
|---------|-------|---------|
| `/maister-fork:init` | `/maister-fork:init [--standards-from=PATH]` | Initialize framework with project analysis and smart defaults for docs/standards. Optionally copy standards from another project's `.maister/docs/standards/` instead of built-in defaults. |
| `/maister-fork:standards-update` | `/maister-fork:standards-update [description] [--from=PATH]` | Update/create standards from conversation context, or sync from another project |
| `/maister-fork:standards-discover` | `/maister-fork:standards-discover [--scope=SCOPE]` | Discover standards from config files and code patterns |

> **Note**: These are all skills (not commands). `/maister-fork:init`, `/maister-fork:standards-update`, and `/maister-fork:standards-discover` invoke their respective skills which delegate file operations to the internal `docs-manager` skill.
...
|---------|-------|----------------|
| `/maister-fork:development` | `[desc] [--research=PATH] [--sequential]` (new) / `[task-path] [--from=PHASE] [--reset-attempts] [--sequential]` (resume) | `.maister/tasks/development/` |
| `/maister-fork:performance` | `[desc] [--sequential]` (new) / `[task-path] [--from=PHASE] [--sequential]` (resume) | `.maister/tasks/performance/` |
| `/maister-fork:research` | `[question] [--type=TYPE] [--brainstorm] [--no-brainstorm] [--design] [--no-design]` (new) / `[task-path] [--from=PHASE]` (resume) | `.maister/tasks/research/` |
| `/maister-fork:product-design` | `[desc] [--research=PATH] [--no-visual]` (new) / `[task-path] [--from=PHASE]` (resume) | `.maister/tasks/product-design/` |
...
| Command | Usage | Purpose |
|---------|-------|---------|
| `/maister-fork:reviews-code` | `[path] [--scope=SCOPE]` | Automated code quality, security, performance analysis |
| `/maister-fork:reviews-pragmatic` | `[path]` | Detect over-engineering, ensure code matches project scale |
| `/maister-fork:reviews-spec-audit` | `[spec-path]` | Independent spec audit for completeness and clarity |
| `/maister-fork:reviews-reality-check` | `[task-path]` | Validate work actually solves the problem |
| `/maister-fork:reviews-production-readiness` | `[path] [--target=ENV]` | Pre-deployment verification with GO/NO-GO recommendation |

### Quick Commands

| Command | Usage | Purpose |
|---------|-------|---------|
| `/maister-fork:quick-plan` | `[task description]` | Enter planning mode with standards awareness from INDEX.md |
| `/maister-fork:quick-dev` | `[task description]` | Implement directly with standards awareness (no planning) |
| `/maister-fork:quick-bugfix` | `[bug description]` | Quick bug fix with TDD red/green gates and complexity escalation |
...
|-------|---------|------------|---------|
| `project-analyzer` | Deep codebase analysis for tech stack, architecture, conventions | `/maister-fork:init` | `agents/project-analyzer.md` |
